...
class GridEnvironment(QWidget):
	...
	def __init__(self, grid_size=128, n_x=5, n_y=5, name="RL Grid Example"):
		...
		for x in range(n_x):
			for y in range(n_y):
				if self.grid_map[x, y] == 1:
					...
					self.grid_reward[x, y] = self.lose_reward
		...
		self.clear_btn.setGeometry(2*self.offset + self.n_x*self.grid_size, 2*self.offset + 64, 128, 64)
		...
		self.lose_reward_label = QLabel("Lose reward: ", parent=self)
		self.lose_reward_label.setGeometry(2*self.offset + self.n_x*self.grid_size, 4*self.offset + 192, 128, 64)
		self.lose_reward_label.setFont(self.font)
		self.lose_reward_line = QLineEdit("{:.2f}".format(self.lose_reward), parent=self)
		self.lose_reward_line.setGeometry(3*self.offset + self.n_x*self.grid_size + 128, 4*self.offset + 192, 128, 64)
		self.lose_reward_line.setFont(self.font)
	...
	def update_reward(self):
		try:
			...
			self.lose_reward = float(self.lose_reward_line.text())
		except:
		...
		for x in range(self.n_x):
			for y in range(self.n_y):
				...
				#Draw hole
					...
					if self.lose_reward > 0:
						c = QColor(0, 255, 0)
					elif self.lose_reward < 0:
						c = QColor(255, 0, 0)
					else:
						c = QColor(255, 255, 255)
				...
				#Draw grid
					...
					if self.grid_value[x, y] < 0:
						if self.lose_reward == 0:
							...
							alpha = self.grid_value[x, y] / self.lose_reward
